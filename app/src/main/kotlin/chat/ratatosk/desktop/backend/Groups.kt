...
class Channel(
    ...
    /**
     * Сколько сейчас живых источников канала (§7.5.1); `null` — канал наш:
     * себе не раздают. Ноль — «никто из достижимых не отдаёт этот канал»,
     * а не «канал пуст» и не «мы без сети».
     */
    val sourcesNow: UInt?,
    /**
     * Сколько годных записей каталога мы знаем (§7.5).
     *
     * Рядом с [sourcesNow] нарочно: «раздавать некому» и «есть кому,
     * а мы не дозвонились» — разные беды, снаружи неотличимые.
     */
    val seedsKnown: UInt,
    /** Объявлено и ещё едет (§7.1, шаг 4): число живое и убывает само. */
    val awaitingBlocks: UInt,
    /** Владельцу: ключу чтения больше месяца (§6.4). */
    val rotationOverdue: Boolean,
    /**
     * Чем объяснить тишину — **один** признак на экран (§15).
     *
     * Выбор главного из фактов делает ядро: повтори мы правило здесь,
     * оно зажило бы в двух местах и разошлось бы на первой правке.
     */
    val signal: org.ratatosk.core.FfiChannelSignal,
    /**
     * Что показывать, пока канал не открылся (§10.5); `null` — ждать
     * нечего. Отсчёт идёт от первой просьбы и повтором не двигается.
     */
    val waiting: org.ratatosk.core.FfiWaiting?,
)
